...
import {
  exchangeWebFlowCode,
  type ExchangeWebFlowCodeGitHubAppOptions,
  type ExchangeWebFlowCodeGitHubAppResponse,
...
class ApiError extends Error {
  ...
  public get responseBuffer(): Buffer {
    return Buffer.from(
      JSON.stringify({ error: this.error, errcode: this.errcode }),
    );
  }
}
...
type StoredAuth = Omit<
  ExchangeWebFlowCodeGitHubAppResponse["data"],
  "expires_in" | "refresh_token_expires_in"
> & { refresh_token_expires_at: number; expires_at: number };

export class CrafterService {
  private static readJSONBuffer(
    req: IncomingMessage,
      ...
      if (!res.headersSent) {
        const error =
          ex instanceof ApiError
            ? ex
            : new ApiError("MC_UNKNOWN", "Unknown error");
  ...
  private async handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    if (url.pathname === "/auth") {
      ...
      if (req.method === "OPTIONS") {
        ...
        const response = this.onAuthExchange(
          (await CrafterService.readJSONBuffer(req)) as any,
        );
        res.setHeader("Content-Type", "application/json");
        res.write(Buffer.from(JSON.stringify(response)));
        res.end();
        ...
        throw new ApiError(
          "MC_WRONG_METHOD",
          "Unexpected method, expected POST",
          405,
        );
      }
      ...
      if (req.method === "POST") {
        const response = await this.onAuthRefresh(
          (await CrafterService.readJSONBuffer(req)) as any,
        );
        res.setHeader("Content-Type", "application/json");
        res.write(Buffer.from(JSON.stringify(response)));
  ...
  private async onAuthCallback(url: URL): Promise<string> {
    ...
    if (!this.stateToCode.delete(state)) {
      throw new ApiError(
        "MC_INVALID_STATE",
        "State not recongised. The request may have timed out",
        401,
      );
    }
