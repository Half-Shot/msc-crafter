...
export async function searchForMSCs(graphql: typeof GraphQL, query: string): Promise<SearchMCSCsResponse["search"]["nodes"]> {
    // N.B We don't stop the user from breaking out of this query, if they want to break their
    // own app then that's fine.
    const {search} = await graphql<SearchMCSCsResponse>(searchMSCsQuery, { searchQ: `repo:matrix-org/matrix-spec-proposals ${query}`});
    return search.nodes;
}