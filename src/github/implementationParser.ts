...
const MatrixJsSdkUrlRegex =
  /https:\/\/github.com\/matrix-org\/matrix-js-sdk\/pull\/(\d+)/g;
...
export function getImplementationsFromString(
  ...
  for (const match of body.matchAll(SynapseShortRegex)) {
    impls.push({
      title: `Synapse #${match[1]}`,
      url: `https://github.com/${match[1]}/synapse/pull/${match[2]}`,
    });
    body = body.replace(match[0], "");
  }
  ...
  for (const match of body.matchAll(MatrixJsSdkShortRegex)) {
    impls.push({
      title: `Matrix JS SDK #${match[1]}`,
      url: `https://github.com/matrix-org/matrix-js-sdk/pull/${match[1]}`,
    });
    body = body.replace(match[0], "");
  }
  ...
  for (const [url, org, repo, prNumber] of body.matchAll(
    GenericGitHubProjectRegex,
  )) {
    // Hide spec repos
    if (
      org === "matrix-org" &&
      ["matrix-spec", "matrix-spec-proposals", "matrix-doc"].includes(repo)
    ) {
      continue;
    }
  ...
  for (const [stringMatch, org, repo, prNumber] of body.matchAll(
    GitHubLinkedPR,
  )) {
    // Hide spec repos
    if (
